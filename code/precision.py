...
def read_file(tsv_file: str) -> Tuple[List, pd.DataFrame]:
    """
    Reads the input 4-column word mover's closeness existing pairs TSV file in a pandas dataframe, generates all the unique PMIDs
    and returns the dataframe.
    Parameters
    ----------
    tsv_file : str
        File path to the 4-column word mover's closeness existing pairs TSV file.
    Returns
    -------
    ref_pmids : list
    ...
    """
    colnames = ["PMID1", "PMID2", "Relevance", "Word mover's closeness"]
...
def sort_collection(pmid: str, data: pd.DataFrame) -> pd.DataFrame:
    """
    Sorts the input dataframe for the given PMID based on the word mover's closeness values in the descending order.
    Parameters
    ----------
    pmid : str
    ...
    sorted_collection = collection.sort_values(['PMID1', "Word mover's closeness"],
...
if __name__ == "__main__":
    ...
    parser.add_argument("-c", "--wmd_file_oath", help="File path to the 4-column word mover's closeness existing pair matrix"
                        , required=True)
